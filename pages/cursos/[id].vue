...
<template>
    ...
    <div class="md:hidden flex flex-row text-main">
        <span v-on:click="showOptions = true">{{ curso.attributes.name }} > </span> <span>{{ selectedCap.name }} > </span> <span>{{ selectedOption.title}}</span>
    </div>

    <ViewsContent class="flex flex-row mb-10 min-h-screen ">
        <div class="md:max-w-xl md:w-1/2 w-full min-h-full text-main md:border-r-2 border-gray-600 " :class="showOptions ? 'md:static static' : ' md:flex-col md:flex hidden' ">
            <h2 class="text-3xl text-clip min-h-16 w-full font-semibold text-black mb-10">{{ (curso.attributes.name as string) }} </h2>
            <Disclosure as="div" v-for="capitulo in curso.capitulos" class="m-2 text-xl" v-slot="{ open }">
                <DisclosureButton class="flex flex-col w-full justify-between text-left font-medium p-2">
                    ...
                    <div class="transition-opacity duration-700 ease-in" :class="open ? 'h-1 w-10 bg-main-2-darker shadow-xl rounded-xl mr-5 opacity-100' : 'opacity-0'"></div>
                </DisclosureButton>
