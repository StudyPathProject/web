    ...
    .then((res) => {
        res.curso.data.attributes.Capitulos.map(async (e) => {
            contenidoDelCurso.push(await getContenidos(id.toString(), e.name)
                .then((res) => {
                    console.log(res.curso.data.attributes.Capitulos[0])
                    return res.curso.data.attributes.Capitulos[0]
                })
            )
        })
...
console.log(curso, contenidoDelCurso)
...
<template>
    <div class="flex md:flex-row flex-col mb-10 min-h-screen md:mx-10 mx-5 mt-10">
        ...
        <div class="md:max-w-sm w-full md:min-h-full text-left min-h-screen text-main md:border-r-2 border-r-0 border-gray-300 "
            ...
            <Disclosure as="div" v-for="capitulo in contenidoDelCurso" class="md:text-xl text-2xl md:mr-5"
                v-slot="{ open }">
        ...
        <div v-if="SelectedCap != null" class="text-left md:w-full text-black w-screen md:mx-10 -mx-5">
            ...
            <div v-for="contenido in SelectedCap['Contenido']" class="flex flex-col w-full mt-10 mb-10 justify-center items-center gap-10">

                <Markdown v-if="contenido.text"
                    class="font-normal leading-relaxed text-xl md:w-full text-left mx-5"
                    :markdownString="contenido.text" />

                <img v-if="contenido.image" v-for="img in contenido.image.data" class="h-full aspect-auto md:w-5/6 w-full rounded-xl shadow-xl"
